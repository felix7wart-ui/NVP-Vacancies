...
CAREER_LINK = re.compile(r"career|carri|vacanc|vacature|jobs?\b|werken[- ]bij|join[- ]us|join[- ]our|werkenbij|/stages?(/|$)|internship|intern-program|team/join", re.I)
# Words on the careers page that suggest an actual opening
OPENING = re.compile(r"internship|intern\b|stagiair|stagiaire|stageplaats|afstudeer|traineeship|werkstudent|praktikum|analyst|associate|vacature|vacancy|vacancies|we are (looking|hiring)|we zoeken|wij zoeken|job opening|open position", re.I)
# Phrases that say there is currently nothing open
NO_OPENING = re.compile(r"no (current |open )?(vacancies|openings|positions)|currently (no|not)|geen (openstaande )?vacatures|momenteel geen", re.I)
...
HEAD = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0 Safari/537.36",
...
def check(name, url):
    ...
    for a in home.find_all("a", href=True):
        ...
        if CAREER_LINK.search(href) or CAREER_LINK.search(label) or label.lower() in ("stage", "stages", "stageplaatsen"):
